...
def cross_val_score2(dtModel,trainSet_X, trainSet_y, bins):
	vret=[]
	blinlistX=[]
	blinlisty=[]
	...
	for i in range(bins):
		...
		txdata=np.array([[1]*trainSet_X.shape[1]])
		tydata=np.array([])
		for j in range(bins-1):
			#print(txdata.shape,blinlistX[(j+i)%bins].shape)
			#print(tydata.shape,blinlisty[(j+i)%bins].shape)
			txdata=np.concatenate((txdata,blinlistX[(j+i)%bins]),axis=0)
			tydata=np.array(list(tydata)+list(blinlisty[(j+i)%bins])) 
		#print("v",vxdata.shape,vydata.shape,"t",txdata.shape,tydata.shape)
		
		txdata=txdata[1:]
		dtModel= dtModel.fit(txdata,tydata)
		y_=dtModel.predict(vxdata)
		#dt_scores.append(mean_squared_error(vydata, y_))
		vret.append(mean_squared_error(vydata, y_))
...
for k in max_depth:
	...
	dtModel=tree.DecisionTreeRegressor(max_depth=k,random_state=1)
...
numof1st=math.ceil(len(trainSet_X)/float(number_of_bin))*(number_of_bin-1) 
dtModel=tree.DecisionTreeRegressor(max_depth=bestDepthDT0,random_state=1)
dtModel= dtModel.fit(trainSet_X[:numof1st], trainSet_y[:numof1st])
y_=dtModel.predict(trainSet_X[numof1st:])
...
plt.xlabel('Value of depth: Validation MSE')
...
plt.xlabel('Value of depth: Training MSE')
plt.ylabel('Cross-Validated MSE')
